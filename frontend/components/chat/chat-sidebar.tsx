...
export function ChatSidebar({ conversations, isLoading }: ChatSidebarProps) {
  ...
  return (
    <div className="w-72 flex-col h-full hidden lg:flex shrink-0"
         style={{
      ...
      <div className="p-4 flex justify-between items-center shrink-0"
        ...
        <span className="font-semibold text-slate-200 text-sm">Chat History</span>
        <button
          onClick={() => router.push('/chat')}
          className="p-2 rounded-lg text-slate-400 hover:text-violet-300 hover:bg-violet-500/10 transition-all border border-transparent hover:border-violet-500/20"
          title="New Chat"
        >
  ...
  return (
    <div
      className={`group relative flex items-start gap-2.5 px-3 py-2.5 rounded-xl cursor-pointer transition-all ${
        isCurrent ? 'text-violet-300' : 'text-slate-400 hover:text-slate-200'
      }`}
      style={{
    ...
    >
      <MessageSquare className={`w-3.5 h-3.5 mt-1 shrink-0 ${isCurrent ? 'text-violet-400' : 'text-slate-600 group-hover:text-slate-400'}`} />
      ...
      <div className="absolute right-2 top-2 opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        {activeMenu === c.id && (
          <div
            ...
            style={{ background: 'rgba(10, 8, 28, 0.95)', border: '1px solid rgba(139, 92, 246, 0.15)' }}
          ...
          >
            ...
            >
              <Pin className="w-3 h-3 text-violet-400" /> {c.is_pinned ? 'Unpin' : 'Pin'}
            </button>
            <button
              onClick={() => {
